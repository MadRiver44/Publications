...
Similar to a linked list, each node is referenced by __only__ one other node, its parent (except for the root node). So we can say that each node in a BST is in itself a BST because further down the tree, we reach another node and that node has a left and a right, then depending on which way we go, that node has a left and a right and so on. 
...
Speed. As mentioned earlier, the BST is an ordered data structure. Upon insertion, the nodes are placed in an orderly fashion. This inherent order, makes searching fast. Similar to binary search, we cut the amount of data to sort thorugh by half on each pass. For example, suppose we are looking for a small node value. On each pass, we keep moving along the left most node, this eliminates half the greater values automatically!
...
```
 ...
 Let's unpack this. 
    ...
    3. if there is a root node, we create a variable, current and set its value to the root node
    ...
    6. if the value is small enough and we reach a point where there is nor more left nodes, we place this item here.
...
Get Min and Get Max. Keep traversing left to the smallest value or right for the largest.
...
In the spirit of concise code, we have a basic traverseDepthFrist function and we pass a function and a method. Again the function implies that we want to do something to the vlaues along the way, the method is the type of search we wish to perform. In the traverseDFS, we have a fallback, preOrder search in place.
...
Check if the BST is full. Remember from earlier, a BST is full if every node has Zero or Two children.
